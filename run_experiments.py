...
MODEL_SIZE = "150M"
MAX_TOKENS = 5_000_000  # Short run (~100 steps at BS=32)

# Experiments to run
EXPERIMENTS = [
    {"batch_size": 32, "lr": 5e-4, "note": "Baseline"},
    {"batch_size": 64, "lr": 5e-4, "note": "Larger Batch"},
    {"batch_size": 128, "lr": 5e-4, "note": "High Throughput"},
    {"batch_size": 256, "lr": 5e-4, "note": "Max VRAM?"},
    {"batch_size": 128, "lr": 1e-3, "note": "High LR"},
]

def run_experiment(config: Dict) -> Dict:
    print(f"\n🧪 Running: BS={config['batch_size']}, LR={config['lr']} ({config['note']})")
...
def main():
    ...
    print("\n" + "="*80)
    print(f"{'Batch':<8} | {'LR':<8} | {'Speed (tok/s)':<15} | {'Loss':<8} | {'Note':<20}")
    print("-" * 80)
    ...
    for r in results:
        print(f"{r['batch_size']:<8} | {r['lr']:<8} | {r['tok_sec']:<15.0f} | {r['loss']:<8.4f} | {r['note']:<20}")
    ...
    print("="*80)
    if best_config:
        print(f"\n🏆 Recommendation: Batch Size {best_config['batch_size']} (Speed: {best_speed:.0f} tok/s)")
